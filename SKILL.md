...
6. **Defense-in-depth, not one boundary.** Layer a rule-based pre-filter before the LLM and a post-filter after; the instruction/data split is necessary, not sufficient.
...
export default {
  ...
  steps: [
    ...
    { id: 'ask', deps: ['audit'], kind: 'llm', config: { messages: [/* data-only, framed as DATA */] } },
...
```
keel run --plan examples/trusted.mjs                       # default provider = Claude Code itself
